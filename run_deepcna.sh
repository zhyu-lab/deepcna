...
python ./detectbp/train.py --input $output_dir/readcounts.txt --epochs 500 --batch_size 256 --lr 0.0001 --latent_dim 2 --seed 0 --output $output_dir >> $log_file 2>&1

echo "Step 3. detect single-cell copy number alterations......"
# the path to MCR v91 needs to be specified
./callcna/run_callcna.sh /usr/local/MATLAB/MATLAB_Runtime/v91 $output_dir/lrc.txt $output_dir/seg.txt $output_dir 10 >> $log_file 2>&1
